...
# Integrity and architecture checks must not resolve caller-controlled tools.
CALLER_PATH=${PATH-}
PATH=/usr/bin:/bin
LC_ALL=C
export PATH LC_ALL
...
安装器不会修改 shell 配置、不会使用 sudo，也不会写入 /usr/local/bin。
EOF
}
...
required_commands='awk chmod cp dirname file find flock grep id ln mkdir mktemp mv readelf readlink rm sed sha256sum stat tr uname wc'
for required_command in $required_commands; do
  command -v "$required_command" >/dev/null 2>&1 || \
    die "缺少安装依赖：$required_command"
done
...
for package_name in $EXPECTED_NAMES SHA256SUMS.txt; do
  case "$package_name" in
    grok-zh|Install-GrokZh.sh) expected_mode=755 ;;
    *) expected_mode=644 ;;
  esac
  [ "$(stat -c '%a' "$SCRIPT_DIR/$package_name")" = "$expected_mode" ] || \
    die "软件包文件权限无效：$package_name（应为 0$expected_mode）"
done
...
ensure_secure_dir() {
  ...
  [ "$(stat -c '%u' "$secure_dir")" = "$(id -u)" ] || die "安装目录不属于当前用户：$secure_dir"
  chmod 700 "$secure_dir" || die "无法保护目录权限：$secure_dir"
  [ "$(stat -c '%a' "$secure_dir")" = 700 ] || die "安装目录权限不是 0700：$secure_dir"
}
...
[ -f "$LOCK_FD_PATH" ] || die "安装锁描述符不是普通文件：$LOCK_FILE"
lock_path_identity=$(stat -Lc '%d:%i' "$LOCK_FILE") || die "无法检查安装锁路径。"
lock_fd_identity=$(stat -Lc '%d:%i' "$LOCK_FD_PATH") || die "无法检查安装锁描述符。"
[ "$lock_path_identity" = "$lock_fd_identity" ] || die "安装锁在打开时发生变化：$LOCK_FILE"
[ "$(stat -Lc '%u' "$LOCK_FD_PATH")" = "$(id -u)" ] || die "安装锁不属于当前用户：$LOCK_FILE"
[ "$(stat -Lc '%a' "$LOCK_FD_PATH")" = 600 ] || die "安装锁权限不是 0600：$LOCK_FILE"
flock -n 9 || die "另一个 grok-zh 安装或自动更新正在进行，请稍后重试。"
[ ! -L "$LOCK_FILE" ] && [ -f "$LOCK_FILE" ] && \
  [ "$(stat -Lc '%d:%i' "$LOCK_FILE")" = "$lock_fd_identity" ] || \
  die "安装锁在加锁时发生变化：$LOCK_FILE"

cleanup() {
  if [ -n "$STAGE_FILE" ]; then
    rm -f -- "$STAGE_FILE" 2>/dev/null || true
  fi
  if [ -n "$FINAL_RESERVATION" ]; then
...
swap_link() {
  ...
  swap_path=$2
  swap_name=${swap_path##*/}
  SWAP_TMP=$(mktemp "$BIN_DIR/.${swap_name}.tmp-link.XXXXXX") || return 1
  rm -f -- "$SWAP_TMP" || return 1
  ln -s "$swap_target" "$SWAP_TMP" || return 1
  if ! mv -Tf "$SWAP_TMP" "$swap_path"; then
    rm -f -- "$SWAP_TMP" 2>/dev/null || true
    SWAP_TMP=
    return 1
  fi
...
MARKER_FILE="$BIN_DIR/.grok-zh-install.json"
...
if [ "$WITH_COMPAT_ALIASES" -eq 1 ]; then
  ...
  grok_alias_managed=0
  agent_alias_managed=0
  if [ -L "$GROK_ALIAS_LINK" ] && [ "$(readlink "$GROK_ALIAS_LINK")" = grok-zh ]; then
    grok_alias_managed=1
  fi
  if [ -L "$AGENT_ALIAS_LINK" ] && [ "$(readlink "$AGENT_ALIAS_LINK")" = agent-zh ]; then
    agent_alias_managed=1
  fi
  if [ "$grok_alias_managed" -eq 1 ] && [ "$agent_alias_managed" -eq 1 ]; then
    WITH_COMPAT_ALIASES=1
  elif [ "$grok_alias_managed" -eq 1 ] || [ "$agent_alias_managed" -eq 1 ]; then
    die "兼容入口状态不完整；请修复 $GROK_ALIAS_LINK 与 $AGENT_ALIAS_LINK 后重试。"
  fi
fi
[ ! -L "$MARKER_FILE" ] || die "拒绝覆盖符号链接安装标记：$MARKER_FILE"
if [ -e "$MARKER_FILE" ]; then
  [ -f "$MARKER_FILE" ] || die "安装标记不是普通文件：$MARKER_FILE"
  [ "$(stat -c '%u' "$MARKER_FILE")" = "$(id -u)" ] || die "安装标记不属于当前用户：$MARKER_FILE"
fi
...
actual_binary_hash=$(sha256sum "$STAGE_FILE" | awk '{ print $1 }')
...
if ! rm -f -- "$STAGE_FILE"; then
  rm -f -- "$FINAL_FILE" 2>/dev/null || true
  die "无法完成已验证版本的原子发布。"
fi
STAGE_FILE=
FINAL_REL="../grok-zh-downloads/${FINAL_FILE##*/}"
MARKER_STAGE=$(mktemp "$BIN_DIR/.grok-zh-install.XXXXXX") || die "无法创建安装标记暂存文件。"
if [ "$WITH_COMPAT_ALIASES" -eq 1 ]; then
  printf '%s\n' '{"product":"grok-build-zh","commands":["grok-zh","agent-zh","grok","agent"]}' > "$MARKER_STAGE"
else
  printf '%s\n' '{"product":"grok-build-zh","commands":["grok-zh","agent-zh"]}' > "$MARKER_STAGE"
fi
...
# Marker rename is the commit point. Ignore termination signals for this tiny
# section so a signal cannot commit the marker and then roll the links back.
trap '' HUP INT TERM
if mv -Tf "$MARKER_STAGE" "$MARKER_FILE"; then
  MARKER_STAGE=
else
  rollback_or_die "入口已激活，但未能提交安装标记"
fi
...
case ":${CALLER_PATH}:" in
  *":$BIN_DIR:"*) ;;
  *)
    printf '%s\n' '请把下面一行加入 shell 启动文件（例如 ~/.bashrc 或 ~/.zshrc）：'
