...
Write all natural-language plan content—including plan titles, step text, update explanations, and progress summaries—in the same natural language as the user's request. If the user's request contains Chinese, or active instructions ask for Chinese, use concise Simplified Chinese and do not translate it into English. Preserve code identifiers, commands, paths, URLs, configuration keys, symbols, product and proper names, task IDs, and canonical status values such as `pending`, `in_progress`, `completed`, and `cancelled` verbatim.
