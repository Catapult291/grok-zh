...
pub fn render_usage_modal(
    ...
    let tab = tab_lines(state, balance, theme, content.width, locale);
...
fn session_info_content(
    ...
    for row in text.lines() {
        ...
        if trimmed.is_empty()
            ...
            || trimmed.starts_with("Run `grok-zh login`")
        {
            continue;
        }
        ...
        let display_label = localized_session_field(locale, label);
        ...
        if compact {
            lines.push(Line::from(vec![
                Span::styled(format!("{display_label}: "), theme.muted()),
                Span::styled(value.to_string(), value_style),
            ]));
        } else {
            let mut label_spans = vec![Span::styled(format!("{display_label}:"), theme.muted())];
            if label == "Session ID" {
                label_spans.push(Span::styled(
                    format!(
                        "   {}",
                        locale.named_static_text(
                            "usage.modal.session.copy_hint",
