...
`grok-zh` 是基于 xAI 官方 Grok Build 源码维护的非官方简体中文社区版本。它尽量保持官方命令、协议和配置格式兼容，但使用独立的程序名与用户数据目录，不会覆盖官方 `grok`。
...
- 可执行文件：`grok-zh.exe`
- 用户数据目录：`~/.grok-zh`
- 覆盖用户目录的环境变量：`GROK_ZH_HOME`
- 运行时内置帮助：`~/.grok-zh/README.md`

官方 `grok` 使用的 `~/.grok` 不属于本社区构建。不要把社区测试包覆盖到官方安装目录，也不要把 `GROK_ZH_HOME` 指向官方数据目录。
...
凭据保存在社区数据目录下，而不是官方 `~/.grok`。不要把凭据文件提交到 Git，也不要在问题报告中粘贴 token、API Key 或完整认证日志。
...
会话、日志、调试信息、缓存和内置文档都写入 `~/.grok-zh`。测试预览包建议设置临时目录：

```powershell
$env:GROK_ZH_HOME = 'C:\temp\grok-zh-test-home'
.\grok-zh.exe
```

测试结束后先退出程序，再决定是否保留该目录。不要在 Grok 任务运行期间移动、删除或编辑会话文件。

会话恢复、云端同步、共享和跨设备能力可能依赖官方服务端；本地 Fork 只能保证客户端侧的路径隔离与协议兼容努力。

## MCP、技能、插件与 Hooks

MCP server、技能、插件和 Hooks 继续使用官方兼容格式：

- 用户级资源放在 `~/.grok-zh` 对应子目录；
...
在独立更新源上线前，请手动下载新的社区测试包，校验 SHA-256，并放在独立目录中测试。
