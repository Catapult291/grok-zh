...
pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grok-zh login`, set XAI_API_KEY, or add api_key to ~/.grok-zh/config.toml.";
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.grok-zh/auth.json".to_string())),
    )
}
